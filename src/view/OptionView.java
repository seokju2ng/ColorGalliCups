...
public class OptionView extends JPanel {
	...
	public OptionView() {
		...
		for (int i = 0; i < 3; i++) {
			name[i].setFont(font);
			name[i].addActionListener(new FocusButtonHandler(this));
		...
		slash = new JLabel[] { new JLabel("/"), new JLabel("/"), new JLabel("/"), new JLabel("/") };
		...
		for (int i = 0; i < btn.length; i++) {
			for (int j = 0; j < btn[i].length; j++) {
				btn[i][j].addActionListener(new FocusButtonHandler(this));
